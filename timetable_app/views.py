    ...
    if request.method == "POST":
        ...
        try:
            ...
            # =====================================
                ...
                theory_rooms = list(
                    Classroom.objects.filter(is_lab=False)
                )
                ...
                RecentActivity.objects.create(
                    title=f"Generated timetable for {department.name}",
                    action_type='Generate'
                )

                messages.success(
                    request,
                    f"{department.name} timetable regenerated."
                )
            ...
            # =====================================
                ...
                messages.success(
                    request,
                    f"{semester} timetable regenerated."
                )
...
from .forms import SubjectFacultyForm,SubjectLabRoomForm
...
# ======================
@login_required
def add_subject_lab_room(request):

    show_list = request.GET.get("view_list") == "true"

    form = SubjectLabRoomForm(
        request.POST or None
    )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            RecentActivity.objects.create(
                title=f"{assignment.subject.name} assigned to Lab {assignment.classroom.room_number}",
                action_type='Lab Room'
            )

            return redirect(
                reverse('add_subject_lab_room')
                + '?view_list=true'
            )
...
@login_required
def delete_subject_lab_room(request, id):
    ...
    RecentActivity.objects.create(
        title=f"Removed lab room {assignment.classroom.room_number} from {assignment.subject.name}",
        action_type='Lab Room'
    )
    ...
    return redirect(
        reverse('add_subject_lab_room') + '?view_list=true'
    )
